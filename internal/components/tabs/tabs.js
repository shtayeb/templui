(function () {
  ...
  function setActiveTab(container, value) {
    ...
    // Update triggers
    container.querySelectorAll(`[data-tui-tabs-trigger][data-tui-tabs-id="${tabsId}"]`).forEach(trigger => {
    ...
    // Update contents
    container.querySelectorAll(`[data-tui-tabs-content][data-tui-tabs-id="${tabsId}"]`).forEach(content => {
    ...
    // Update marker
    const marker = container.querySelector(`[data-tui-tabs-marker][data-tui-tabs-id="${tabsId}"]`);
  ...
  function initializeTabs() {
    document.querySelectorAll('[data-tui-tabs]').forEach(container => {
      ...
      const triggers = container.querySelectorAll(`[data-tui-tabs-trigger][data-tui-tabs-id="${tabsId}"]`);
      if (triggers.length === 0) return;
      
      // Find default trigger - check for active state or use first
      const defaultTrigger = Array.from(triggers).find(t => 
        t.getAttribute('data-tui-tabs-state') === 'active'
      ) || triggers[0];
      
      if (defaultTrigger) {
        setActiveTab(container, defaultTrigger.getAttribute('data-tui-tabs-value'));
      }
    });
  }
  ...
  new MutationObserver(initializeTabs).observe(document.body, { childList: true, subtree: true });
})();